...
exports.default = strapi_1.factories.createCoreService('api::content-distribution.content-distribution', ({ strapi }) => ({
    async executeDistribution(ruleId) {
        try {
            ...
            // 更新统计信息
            const statistics = rule.statistics || {
                totalRuns: 0,
                successfulRuns: 0,
                failedRuns: 0,
            ...
            statistics.successfulRuns += 1;
            statistics.lastSuccessfulRun = new Date();
            statistics.totalArticlesDistributed += results.distributed;
            // 更新规则状态
    ...
    async distributeArticles(articles, rule) {
    ...
    async checkIfAlreadyDistributed(articleId, targetSites) {
    ...
    applyTransformations(article, transformations) {
        if (!transformations) {
            return article;
        }
        ...
        if (transformations.addTags && transformations.addTags.length > 0) {
            ...
            const newTags = transformations.addTags.filter(tag => !existingTags.some(existingTag => existingTag.name === tag));
    ...
    async distributeToSite(article, siteId, ruleId) {
        try {
            // 获取目标站点信息
    ...
    async createDistributionRecord(articleId, siteId, ruleId, status, errorMessage) {
        const recordService = strapi.service('api::distribution-record.distribution-record');
