...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    try {
      ...
      let site = (await strapi.entityService.findMany('api::site.site', {
      ...
      for (const a of extraArticles) {
        const exist = (await strapi.entityService.findMany('api::article.article', {
          filters: { slug: a.slug },
          limit: 1,
        }))?.[0];
        if (!exist) {
      ...
      for (const s of moreSitesPayload) {
        let found = (await strapi.entityService.findMany('api::site.site', {
      ...
      const moreCategories: any[] = [category];
      for (const c of moreCategoriesPayload) {
        let found = (await strapi.entityService.findMany('api::category.category', {
          filters: { slug: c.slug },
          limit: 1,
        }))?.[0];
        if (!found) {
      ...
      const moreTags: any[] = [tag];
      for (const t of moreTagsPayload) {
        let found = (await strapi.entityService.findMany('api::tag.tag', {
          filters: { slug: t.slug },
          limit: 1,
        }))?.[0];
        if (!found) {
      ...
      const tagToArticleSlugs: Record<string, string[]> = {
      ...
      for (const [tagSlug, articleSlugs] of Object.entries(tagToArticleSlugs)) {
        const tagRecord = (await strapi.entityService.findMany('api::tag.tag', {
        ...
        const articleDocIds: string[] = [];
        for (const s of articleSlugs) {
          const art = (await strapi.entityService.findMany('api::article.article', {
            filters: { slug: s },
            limit: 1,
          }))?.[0];
          if (art) {
            articleIds.push(Number((art as any).id));
            if ((art as any).documentId) articleDocIds.push(String((art as any).documentId));
          }
        }
        ...
        if (articleIds.length || articleDocIds.length) {
          try {
            ...
            await (strapi as any).documents('api::tag.tag').update({
              documentId: (tagRecord as any).documentId,
            ...
            await strapi.entityService.update('api::tag.tag', tagRecord.id, {
              data: { articles: articleIds as any },
            });
          }
        }
      }
